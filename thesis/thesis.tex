...
Runtime of each method is an important factor to considered. Some methods are not very scalable for large datasets as already discussed in chapter \ref{chap:rw}. It is not easy to describe the metric learning algorithms in big O notation because most of them are iterative and it is not clear how many iterations is needed. Plus they can still end up in some local minima so multiple runs may be necessary for optimal results.

In this experiment we measured the runtime of all methods (the same methods as in previous experiments): Covariance, LMNN, LFDA, NCA, CMAES.kNN, CMAES.fMe, jDE.kNN and jDE.fMe. All methods were run 10 times on every dataset with the best hyper-parameters determined from the first experiment in section  \ref{chap:exp:classification} which corresponds to best error rate for given dataset as shown in table \ref{tab:errors-small}. The results are in table \ref{tab:runtimes} where the runtimes shown are averaged across all 10 runs. Euclidean distance does not need any metric training and therefore has a zero runtime in the table. Covariance metric is the seconds fastest metric, closely followed by LFDA algorithm. On the other hand, LMNN and NCA metrics and all evolution metrics are the slowest methods.
...
\cenfig{graphs/runtime/fixed-dim}{Runtimes for increasing number of samples with a fixed dimension (normal and log scale)}{fig:runtime:fixed-dim}

\cenfig{graphs/runtime/fixed-samples}{Runtimes for increasing dimension with a fixed number of samples (normal and log scale)}{fig:runtime:fixed-samples}
