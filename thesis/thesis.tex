...
\subsection{Evolutionary strategy (ES)}
...
\begin{table}[ht] \label{tab:datasets} \centering
\begin{tabular}{lrrr}
\hline
...
\hline
\end{tabular}
\caption{Small UCI datasets summary}
\end{table}
...
Table \ref{tab:datasets} summarizes small datasets and table \ref{tab:big-datasets} summarizes large datasets that we experimented on. Both table show number of samples, dimensionality and number of distinct classes for each dataset.

[Short info about all datasets we used? Multivariate, Real / Integer / Categorical, Missing data? %TODO ]

\begin{table}[ht] \label{tab:big-datasets} \centering
\begin{tabular}{lrrr}
\hline
% \multicolumn{4}{c}{Item} \\
% \cline{1-2}
...
\hline
\end{tabular}
\caption{Larger datasets summary}
\end{table}
