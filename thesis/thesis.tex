...
Several general purpose metrics exist, such as Euclidean distance, defined in equation \ref{eq:euklid}, or its generalized Minkowski distance, defined in equation \ref{eq:minkowski}, and cosine similarity, defined in equation \ref{eq:cosine} used for vectors of real numbers, or Levenshtein distance (also called edit distance) for measuring distances between strings.

% https://numerics.mathdotnet.com/distance.html
\begin{align}
d_{\mathbf{2}}(\textbf{x}, \textbf{y}) &= \|\textbf{x}-\textbf{y}\|_2 = \sqrt{\sum_{i=1}^{n} (x_i-y_i)^2} \label{eq:euklid} \\
...
Many researches tackle metric learning by adapting a Mahalanobis distance metric first described in \cite{mahalanobis1936generalized} in \ref{eq:mahalanobis}, where $\bm{\Omega}$ is a covariance matrix of the data. However this name is now overloaded and Mahalanobis distance now stands for any metric parametrized by a matrix $\bm{M}$ as defined in equation \ref{eq:mahalanobis:generalized}.

\begin{equation} \label{eq:mahalanobis}
d_{Mahalanobis}(\textbf{x},\textbf{y}) = \sqrt{(\textbf{x}-\textbf{y})^T\bm{\Omega}^{-1}(\textbf{x}-\textbf{y})} 
\end{equation}

\begin{equation} \label{eq:mahalanobis:generalized}
d_{Mahalanobis}(\textbf{x},\textbf{y}) = \sqrt{(\textbf{x}-\textbf{y})^T\bm{M}(\textbf{x}-\textbf{y})} 
\end{equation}

\section{Mahalanobis metric}

From a mathematical perspective, a metric is a function $d(x,y)$ that defines a distance, similarity or dissimilarity between two instances from some input space: $d:\mathcal{X} \times \mathcal{X} \mapsto [0,\inf)$. For a function to be a metric it has to follow 4 rules: nonnegativity, identity of indiscernibles, symmetry and triangle inequality as defined in equations \ref{eq:metricdef-1} ---\ref{eq:metricdef-4}.
...
In order for Mahalanobis metric from equation \eqref{eq:mahalanobis:generalized} to be a metric, the matrix $\bf{M}$ that parametrizes the distance needs to pass the metric axioms \ref{eq:metricdef-1} ---\ref{eq:metricdef-4}. Therefore the axiom identity of indiscernibles \ref{eq:metricdef-2} is too strong and is usually relaxed into an axiom of identity \ref{eq:pseudodef-2}. Together with the rest of the axioms, this gives us a set of new axioms \ref{eq:pseudodef-1} ---\ref{eq:pseudodef-4}, which together define a pseudo-metric.

$\bm{M} \in \mathbb{S}^{d \times d}_+$ ensures that $d_{\bm{M}}$ satisfies the properties of a pseudo-metric: $\forall \textbf{x},\textbf{y}, \textbf{z} \in \mathcal{X}$

%TODO fix corrent formatting for letters
\begin{align}
d(x,y) &\geq 0 & nonnegativity \label{eq:pseudodef-1} \\
d_M(x,x) &= 0 & identity \label{eq:pseudodef-2} \\
d(x,y) &= d(y,x) & symmetry \label{eq:pseudodef-3} \\
d(x,z) &\leq d_M(x,y) + d_M(y,z) & triangle \ inequality \label{eq:pseudodef-4}
\end{align} 

Pseudo-metric is a function which satisfies the axioms for a metric, except the second rule (identity of indiscernibles) is slightly relaxed so that it is possible that there $\exists \textbf{x} \neq \textbf{y}: d_M(\textbf{x},\textbf{y}) = 0$.
...
%TODO explain space transformation and mahalanobis metric
...
\begin{align}
  d_{\bm{M}}(\textbf{x},\textbf{y}) &= \sqrt{(\textbf{x}-\textbf{y})^{T}\bm{M}(\textbf{x}-\textbf{y})} \nonumber\\
         ...
         &= d(\bm{L}\textbf{x}, \bm{L}\textbf{y}) \label{eq:mahalanobis:transform}
\end{align}
