...
Line \ref{alg:knn:dist}

\begin{figure}
\begin{algorithm}[H]
\DontPrintSemicolon
\LinesNumberedHidden
\KwData{$\bm{X}$ training data, $\bm{Y}$ class labels of $\bm{X}$, $\bm{x}$ unknown sample}
\KwResult{class label of the unknown sample $\bm{x}$}
\Begin{
  \For{$i\leftarrow 1$ \KwTo $m$}{
\lnl{alg:knn:dist}    distances[i] $\leftarrow$ compute distance d($\bm{X}_i$, $\bm{x}$)\;
  }
  return majority class from $k$ lowest distances\;
}
\caption{Pseudocode for k-NN classification}
\end{algorithm}
\end{figure}
