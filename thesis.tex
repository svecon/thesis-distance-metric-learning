...
Most of the research has been focused on learning this metric from labelled training instances (supervised setting), however researchers also focus on weakly-supervised tasks, where the labels of the instances are unknown, but the information about the data comes in the form of similar and dissimilar pairs (Equations \ref{eq:similar}, \ref{eq:dissimilar}) and semi-supervised setting has received some attention as well.

% Metric learning has mostly been used to improve the performance of \ac{knn} classification.
...
% \begin{array}{rl}{\displaystyle \min _{{x^{1},\ldots ,x^{n}\in {\mathbb  {R}}^{n}}}}&{\displaystyle \sum _{{i,j\in [n]}}c_{{i,j}}(x^{i}\cdot x^{j})}\\{\text{subject to}}&{\displaystyle \sum _{{i,j\in [n]}}a_{{i,j,k}}(x^{i}\cdot x^{j})\leq b_{k}\qquad \forall k}.\\\end{array}
...
\begin{align}
\mathcal{S} &= \lbrace(x_i,x_j): y_i = y_j \text{ and } x_j \text{ is in the }k\text{-neighbourhood of } x_i \rbrace \\
\mathcal{R} &= \lbrace(x_i,x_j,x_k): (x_i,x_j) \in \mathcal{S}, y_i \neq y_k \rbrace
\end{align}

\begin{align}
\min_{\bm{M} \in \mathbb{S}^{d}_+} \quad & (1-\mu)\sum_{(x_i,x_j)\in \mathcal{S}} d_{\bm{M}}^2(x_i,x_j) + \mu \sum_{i,j,k} \xi_{ijk} \\
\text{s.t.} \quad & d_{\bm{M}}^2(x_i,x_k) - d_{\bm{M}}^2(x_i,x_j) \geq 1 - \xi_{ijk} \quad \forall(x_i,x_j,x_k) \in \mathcal{R}
\end{align}
...
\begin{align}
\widetilde{\bm{S}}^{(w)} &= \frac{1}{2} \sum_{i,j=1}^n \widetilde{\bm{W}}_{i,j}^{(w)} (x_i-x_j)(x_i-x_j)^\top
\\
\widetilde{\bm{S}}^{(b)} &= \frac{1}{2} \sum_{i,j=1}^n \widetilde{\bm{W}}_{i,j}^{(b)} (x_i-x_j)(x_i-x_j)^\top
\\
...
\\
\bm{L}_{LFDA} &= \argmax_{\bm{L} \in \mathbb{R}^{d \times r}} \bigg[\tr\Big( ( \bm{L}^\top\widetilde{\bm{S}}^{(w)}\bm{L} )^{-1} \bm{L}^\top\widetilde{\bm{S}}^{(b)}\bm{L} \Big)\bigg] 
\end{align}
...
% \include{results/stat-significance_classification}
% \include{results/stat-significance_dimred}
