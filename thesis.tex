...
The final difference is that we use a \acf{cmaes} to evolve the individuals. This algorithm was introduced in \cite{hansen1996adapting} and it was further improved \citep{hansen2001completely} \citep{hansen2003reducing}, compared  and reviewed to other methods \citep{hansen2006cma}. It is a stochastic algorithm for optimizing non-linear and non-convex black-box optimization problems. In our experiments we use the most common version $(\mu_W, \lambda)$-\ac{cmaes}, described in \cite{hansen2001completely}, where in each iteration step a weighted combination of the μ best out of λ new candidate solutions is used to update the distribution parameters.

We also use a concept called \acf{hof}, which is updated with the best individuals after every generation and stores only the best ones. After the evolution ends, the \ac{hof} contains the best individuals across all generations.

%TODO mention CMAES and describe it in more detail in the next chapter
%TODO describe the whole workflow
