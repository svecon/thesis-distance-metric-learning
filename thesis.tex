...
%TODO "nekde v diplomce mi pripadalo, ze pisescholesky rozklady matice, ktera je low rank a to mam pocit, ze nejde ze musi byt regularni, aby  ho slo udelat"
...
%TODO REWRITE AFTER EVERYTHING IS DONE
...
%TODO check if it is the same in http://www.deeplearningbook.org/contents/linear_algebra.html
\begin{equation} 
...
%TODO PCA is connected to Mahalanobis!!
...
%TODO [add how tsne works?]

%TODO formally explain that evolving 2*D matrix corresponds to some eigen value decomposition???
...
%TODO mention CMAES and describe it in more detail in the next chapter
...
% https://github.com/DEAP/deap/blob/master/deap/algorithms.py#L442
% https://scholar.google.cz/scholar?q=Igel+2007&btnG=&hl=en&as_sdt=0%2C5
% http://www.mitpressjournals.org/doi/pdfplus/10.1162/evco.2007.15.1.1
% https://www.lri.fr/~hansen/cmaartic.pdf
...
%TODO use letter "m"?
%TODO is the claim about Mahalanobis metric correct?????
...
Comparing the \ac{jde} and \ac{cmaes}, we see that the type of fitness (\ac{fme} or \ac{knn}) affects the performance more than the optimization algorithm. However, \ac{jde} has an advantage in the population size, therefore we believe \ac{cmaes} is overall better then \ac{jde} for this task, as it has similar performance with much lower number of fitness evaluations. It is interesting to note that both \ac{jdefme} and \ac{jdeknn} won on the artificial \dataset{gaussians} dataset, however they were closely followed by \ac{cmaesknn} method. \ac{cmaesfme} and \ac{jdefme} methods perform the worst on most of the datasets which suggests that \ac{fme} fitness function does not work very well to improve classification, whereas \ac{knn} behaves much better as a fitness function when compared to \ac{cmaesknn} and \ac{jdeknn}.
