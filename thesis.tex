...
\chapter{Improving Evolutionary DML} \label{chap:our-method}

In this work, we extend upon the research of \cite{fukui2013evolutionary} and we also use \aclp{ea} to evolve a real-valued matrix for Mahalanobis metric, nevertheless, there are notable differences between our methods which we will describe in this chapter.

The work of \cite{fukui2013evolutionary} was focused on improving clustering, whereas our method focuses on classification and dimensionality reduction. To improve classification, a fitness function designed for this purpose should be optimized. The function should be fast because in every generation it will be evaluated number of times proportionally to the size of the population.

In our experiments we use \acf{knn} classifier to measure the performance and so we propose to use a \acf{knn} classifier also as the fitness function. It is relatively fast, it takes $N\log{N}$ to build the underlying k-d tree structure with $N$ training samples and then $M \log{N}$ to classify $M$ testing samples. Optimizing the \ac{knn} classifier inside of the evolution to find a good metric should also directly translate to improving the performance of the final \ac{knn} classifier. It depends on how well the evolution algorithm will be able to find a Mahalanobis metric that will be able to generalize.

The pseudocode describing how \ac{knn} classifier is used as a fitness function is shown In Figure \ref{pseudo:ea:knn}. First, the labelled samples are split into training and testing sets. It is important to note that the splitting is done for each fitness evaluation and therefore the sets are split differently for each individual. The idea behind this is we would like the evolution to find individuals that generalize well. Secondly, Mahalanobis distance is decoded from the individual and a \ac{knn} classifier with this metric is initiated and trained using the training data. Then the classifier is used to predict labels of the testing data and an accuracy of this test is returned as a fitness value of the individual.
%TODO is the splitting argument true?
...
\LinesNumbered
\KwData{set of labelled instances $\mathcal{Z}=\{\bm{z}_i=(\bm{x}_i, y_i): \bm{x}_i \in \mathcal{X}, y_i \in \mathcal{Y}\}^n_{i=1}$, where $\mathcal{X}$ are training data and $\mathcal{Y}$ are their class labels}
\KwIn{the individual $\bm{i}$ encoding the Mahalanobis metric}
\KwResult{the fitness value of the $\bm{i}$ individual}
\Begin{
  Split instances into train-test sets\;
  Initialize \ac{knn} classifier with a Mahalanobis metric encoded in the individual $\bm{i}$\;
...
\end{algorithm} 
\caption{Pseudocode for \ac{knn} as fitness in \ac{ea}} \label{pseudo:ea:knn}
\end{figure}

Second distinction in our work compared to \cite{fukui2013evolutionary} is that instead of evolving the Mahalanobis matrix $\bm{M}$ directly, we evolve a matrix $\bm{L}$ such that $\bm{M}=\bm{L}^\top\bm{L}$. By evolving the matrix $\bm{L}$ we are guaranteed that the matrix $\bm{M}$ is \acf{psd} as proven in Equation \ref{eq:mah:proof}. Moreover if $\bm{L}$ is non-singular then $\bm{M}$ is \acf{pd}. We do not have to evolve the full matrix with $d\times d$ parameters, but it is enough to only evolve a triangular matrix $\bm{L}$ with $d(d+1)/2$ parameters. Also, $\bm{L}$ can be forced to be diagonal, which corresponds to scaling the original space alongside the axis, but it does not account for correlation between the dimensions.

%TODO where to move this PSD definition?
\begin{equation}
...
The final difference is that we use a \acf{cmaes} to evolve the individuals. This algorithm was introduced in \cite{hansen1996adapting} and it was further improved in \cite{hansen2001completely} and \cite{hansen2003reducing}; it was also extensively reviewed and compared to other methods \citep{hansen2006cma}. It is a stochastic algorithm for optimizing non-linear and non-convex black-box optimization problems. In our experiments we use the most common version $(\mu_W, \lambda)$-\ac{cmaes}, described in \cite{hansen2001completely}, where in each iteration a weighted combination of $\mu$ best new candidates (out of $\lambda$) are used to update the distribution parameters.

The main advantage of using \ac{cmaes} over \ac{jde}, which was described earlier, is that \ac{cmaes} needs smaller population size to still work well. Evaluation of the fitness function is very time consuming in our setting and therefore the size of the population directly influences training time. Recommended population size for \ac{cmaes} is $4+\floor{3\log{D}}$ \citep{hansen2006cma} whereas for \ac{jde} it is $10D$ \citep{brest2006self}, where $D$ is number of parameters to evolve. This makes \ac{jde} intractable for learning Mahalanobis distance in high-dimensional space, because it has to evaluate $10\cdot d(d+1)/2 \approx 5d^2$ ($d$ is dimensionality of the data) fitnesses in each generation, while \ac{cmaes} only needs to evaluate $4+\floor{3\log{d^2}} \approx 4+\floor{6\log{d}}$ fitnesses. We performed an experiment to verify that \ac{jde} algorithm does not perform well with the same population size as \ac{cmaes}. The experiment is discussed in Section \ref{chap:exp:logjde}.

We also use a concept called \acf{hof}, which acts as a storage of the best individuals. It is updated with the best individuals after every generation and only keeps the best ones. After the evolution ends, the \ac{hof} contains the best individuals across all generations.

The high level pseudocode for evolving Mahalanobis distance is shown in Figure \ref{pseudo:our-method}. First, all parameters are initialized along with an empty \acl{hof}. Then the evolution is run until a stopping criterion is met (e.g. number of generations, convergence tolerance...). In each generation new individuals are sampled from Multivariate normal distribution with the current parameters and the fitnesses of the individuals are evaluated. The \ac{hof} and the \ac{cmaes} are updated. After the evolution is finished, the best individual from the \ac{hof} is returned. The individual contains the weight of the transformation function $\bm{L}$.
...
\Begin{
  ...
  Initialize empty \acl{hof}\;
  \Repeat{a termination criterion is met}{
    Sample new individuals using the current \ac{cmaes} parameters\;
    Evaluate fitnesses of the new individuals as described in Figure \ref{pseudo:ea:knn}\;
    Update best individuals in \acl{hof}\;
    Update \ac{cmaes} parameters\;
  }
  \Return the best individual from the \acl{hof}\;
}
\end{algorithm} 
\caption{Pseudocode for evolving Mahalanobis distance using \ac{cmaes}} \label{pseudo:our-method}
\end{figure}
...
We performed five different experiments to assess the performance of metric learning algorithms. In Section \ref{chap:exp:classification} we use learnt metric in \ac{knn} classificator and compare classification errors on various datasets, which are described in \ref{chap:exp:datasets}. In the next experiment in Section \ref{chap:exp:fitness} we compare different evolutionary algorithms together with various fitnesses and show how well they evolve and generalize, while in Section \ref{chap:exp:logjde} the performance of different population sizes of \ac{jde} is compared. In Section \ref{chap:exp:learning-times} we measure the training times of the \ac{dml} algorithms and in the last experiment in Section \ref{chap:exp:dimred} we use metric learning algorithms for visualisation and we compare the resulting embeddings with more traditional methods.

\section{Experimental settings} 
In our experiments we compare metric learning algorithms listed in Chapter \ref{chap:rw}. \ac{lmnn}, \ac{nca}, \ac{lfda} metric learning algorithms were already implemented in an open-sourced Python \textit{metric-learn} library\footnote{\url{https://github.com/all-umass/metric-learn}}. Only our evolutionary method described in Section \ref{chap:our-method} and \ac{jdefme} from  \cite{fukui2013evolutionary} were missing in this library and thus we designed a modular interface and implemented these two methods as a part of the \textit{metric-learn} library. The~implementation is described in chapter \ref{chap:impl}.
