...
The \acl{fme} index performed the best in their experimental results and so we chose to further experiment with only this index. \acl{fme} is a harmonic mean of the precision and recall.

%TODO validity measures
%http://www.cs.kent.edu/~jin/DM08/ClusterValidation.pdf

\begin{align*}
wFme(C) &= \sum_{s \ in T} \frac{N_s}{N} \max_{C_i \in C} F(s,C_i) \\
F(s,C_i) &= \frac{2 \cdot Prec(s,C_i) \cdot Rec(s, C_i)}{Prec(s,C_i) + Rec(s, C_i)} \\
Prec(s,C_i) &= \widetilde{N}_{s,i}/\widetilde{N}_i \\
Rec(s,C_i) &= \widetilde{N}_{s,i}/N_s \\
N_{s,i} &= \#\{x_k | t(k)=s, c(k)=C_i\} \\
N_s &= \#\{x_k | t(k)=s \in T\} \\
N_i &= \#\{x_k | c(k)=C_i\} \\
\widetilde{N}_{s,i} &= \sum_{C_j in C} h_{i,j}N_{s,j} \\
\widetilde{N}_i &= \sum_{s\in T}\widetilde{N}_{s,i}=\sum_{s\in T}\sum_{C_j \in C} h_{i,j}N_{s,j} \\
\widetilde{N} &= \sum_{C_i \in C}\widetilde{N}_i = \sum_{C_i \in C}\sum_{s\in T}\sum_{C_j \in C} h_{i,j}N_{s,j} \\
\end{align*}
